...
let dependentsOfDependency = (sourceFiles, file) => {
  ...
  if (sourceFilesThatImportFile.length) {
    return sourceFilesThatImportFile.reduce((array, sourceFileThatImportFile) => {
      let foundParents = dependentsOfDependency(sourceFiles, sourceFileThatImportFile).filter(
        (parent) => {
          return !array.find((item) => item.path == parent.path)
        },
      )
      return [...array, ...foundParents]
    }, [])
  } else {
    return [file]
  }
